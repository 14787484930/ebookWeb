    ...
    methods: {
      ...
      initDateType() {
        this.$picker.timePicker((val, index, text) => {
          _that.teach.startTime = _that.$toDate(val, 'yyyy-MM-dd HH:mm');
        });
        this.$picker.timeEndPicker((val, index, text) => {
          _that.teach.endTime = _that.$toDate(val, 'yyyy-MM-dd HH:mm');
        });
      },
      initData() {
        this.$http.post('/tutoring/getById/' + this.teach.id).then(res => {
          for (const i in this.teach) {
      ...
      saveData() {
        ...
        this.$save(url, this.teach, '', msg => {
          ...
          if (msg.data.code === 100) {
            ...
            const aMsg = msg.data.page.errors;
            const akey = Object.keys(msg.data.page.errors)[0]
      ...
      // 类型选择
      changeTab(index) {
        const teachTab = document.getElementsByClassName('teach-tab');
        this.flag = index;
