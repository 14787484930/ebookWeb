  ...
  <div>
    <div id='head'>
      <div class='bar bar-header item-input-inset'>
        ...
        <router-link v-if='powerFlag' :to="{name:' teachAdd', query:{id:0}}"
    ...
    methods: {
      ...
      initType() {
        ...
        this.$picker.timePicker((val, index, text) => {
          that.queryList.startTime = that.$toDate(val, 'yyyy-MM-dd HH:mm')
        });
        this.$picker.timeEndPicker((val, index, text) => {
          that.queryList.endTime = that.$toDate(val, 'yyyy-MM-dd HH:mm')
        });
